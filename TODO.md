...
- [ ] **Export Preview**: The `ExportPreview.tsx` component defined in SPEC is missing. Users transition directly from selection to export.
- [ ] **Claude Metadata**: `memories.json` and `users.json` are not parsed or included in the export data model.
- [ ] **Tagging UI**: The `TaggingSetup` screen does not implement the explicit model download flow ("Download now? y/n") described in SPEC.
- [ ] **Test Fixes**:
  - `src/providers/samples.test.ts` fails due to `ReferenceError: __dirname is not defined` (ESM issue).
...
- [ ] **App.tsx Logic**: The `App` component is a "God Object". Refactor routing and state into `AppRouter` or custom hooks (e.g., `useAppFlow`).
- [ ] **CLI Duplication**: `src/cli.ts` duplicates export pipeline logic from `App.tsx`. Extract a shared `ExportManager` or service.
